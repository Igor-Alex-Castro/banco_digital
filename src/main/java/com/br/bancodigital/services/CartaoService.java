...
public class CartaoService {
	...
	public CartaoDTO salvar(CartaoDTO cartaoDto) {
		...
		if(conta.getCartao() != null){
			throw new BusinessException("Esta conta já possui um cartão. Para modificar algo, atualize o cartão existente");
		}
	...
	public DetalhesCartaoDTO obterCartao( Long id) {
		...
		Cartao cartao = cartaoRepository.findById(id).orElseThrow(
				() -> new ResourceNotFoundException("Este cartão não existe"));
		
		Conta conta = contaRepository.findByCartao(cartao).orElseThrow(
				() -> new ResourceNotFoundException("Esta cartão não esta vinculado a nenhuma conta"));;
		
				return new DetalhesCartaoDTO(conta, cartao);
	}	
	
	public DetalhesCartaoDTO limite(Long id, LimiteDto limiteDto) {
